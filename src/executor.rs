...
impl ConsoleModeGuard {
    fn new(handle: HANDLE) -> std::io::Result<Self> {
        ...
        let new_mode = original | ENABLE_EXTENDED_FLAGS;
        let modified = false;

        // disable the console mode change here
...
impl ConPtyHandle {
    fn new(cols: i16, rows: i16, input_read: HANDLE, output_write: HANDLE) -> Result<Self> {
        ...
        let hr = unsafe { CreatePseudoConsole(size, input_read, output_write, 0, &mut handle) };
        if hr != 0 {
            bail!(
